<script lang="ts">
	...
	let autoToggleInterval: ReturnType<typeof setInterval> | undefined;
	let formattedTime = "";
...
<header class="bg-transparent py-3">
	<div class="container mx-auto px-6">
		<div class="grid grid-cols-[auto_auto_1fr] items-center gap-x-3">
			...
			{#if !isMobile}
				<button type="button" {...dropdownTrigger} use:dropdownTrigger class="flex items-center justify-between gap-x-1.5 cursor-pointer">
				...
				{#if $isDropdownOpen}
					<div class="absolute z-50 mt-3 min-w-52 rounded-lg bg-outer-space-100 py-3 shadow-lg" {...dropdownMenu} use:dropdownMenu in:fly={{ y: -10, duration: 300, easing: backOut }} out:fly={{ y: -10, duration: 400, easing: cubicOut, delay: 20 }}>
						{#each locations as location}
							<button type="button" class="block w-full p-3 text-left text-sm/normal font-medium text-outer-space-900 cursor-pointer" {...dropdownItem} use:dropdownItem on:click={() => selectLocation(location)}>
								{location}
							</button>
						{/each}
					</div>
				{/if}
			{:else}
				<button type="button" {...dialogTrigger} use:dialogTrigger class="flex items-center justify-between gap-x-1 cursor-pointer">
					<span class={`font-semibold text-fuscous-gray-900 ${getLocationTextSizeClass(selectedLocation)}`}>
						{selectedLocation}
					</span>
					<svg xmlns="http://www.w3.org/2000/svg" class="size-4 stroke-fuscous-gray-900" viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
					...
					<div class="fixed inset-x-0 bottom-0 z-50 w-full rounded-t-3xl bg-outer-space-100 p-6" {...dialogContent} use:dialogContent in:fly={{ y: 355, duration: 350, easing: quintOut }} out:fly={{ y: 355, duration: 500, easing: quintIn, delay: 60 }}>
						<div class="flex flex-col gap-y-3">
							<div class="flex flex-col">
								{#each locations as location}
									<button type="button" class="block w-full py-3 text-left text-lg/normal font-medium text-outer-space-900 cursor-pointer" on:click={() => selectLocation(location)}>
										{location}
									</button>
								{/each}
							</div>
			...
			<div class="justify-self-end relative flex items-center gap-x-3">
				...
				<button type="button" class="relative cursor-pointer" on:click={togglePickups} {...$tooltipTrigger} use:tooltipTrigger aria-label={$isPickups ? "Disable pickups mode" : "Enable pickups mode"}>
					{#if $isPickups}
			...
			{#if $isTooltipOpen}
				<div {...$tooltipContent} use:tooltipContent transition:fade={{ duration: 100 }} class="z-20 rounded-md bg-old-gold-500 shadow">
					<div {...$tooltipArrow} use:tooltipArrow></div>
					<div class="p-3 flex items-center gap-x-3">
						...
						<button class="relative inline-block w-11 h-5 cursor-pointer" {...timeSwitchRoot} use:timeSwitchRoot>
							<input
								id="switch-component"
								type="checkbox"

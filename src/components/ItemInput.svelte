	...
	<div class="container mx-auto px-6">
		<div class="flex flex-col justify-center my-6 relative">
			<div class="flex w-full items-center justify-between gap-x-6">
				<div class={showInput ? "flex w-full items-center justify-between overflow-hidden rounded-full bg-fuscous-gray-500 p-3 transition-[width] duration-200 ease-in-out" : "relative w-24 rounded-full bg-fuscous-gray-500 px-12 py-7 font-semibold text-fuscous-gray-50 transition-[width] duration-200 ease-in-out"}>
					<button class={showInput ? "mr-3 rounded-full bg-fuscous-gray-600 px-3 py-1 text-base/normal font-semibold text-fuscous-gray-50 transition-all duration-100 ease-[cubic-bezier(0.25,0.8,0.5,1)] cursor-pointer" : "absolute inset-0 cursor-pointer"} on:click={toggleInputVisibility}>
						{#key showInput}
							{#if showInput}
					...
					{#if showInput}
						...
						<button type="button" class="flex items-center justify-center rounded-full bg-fuscous-gray-600 p-1 transition-transform duration-200 ease-in-out cursor-pointer" on:click={addToList} aria-label="Add stop">
				...
				{#if !showInput}
					<ScreenshotButton class="flex items-center justify-center rounded-full bg-old-gold-500 p-4 transition-transform duration-200 ease-in-out cursor-pointer" targetSelector="#capture-target">
